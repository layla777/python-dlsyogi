...
def hash_to_index(hash_code):
    return ((hash_code & 0xffffffff) ^ ((hash_code >> 32) & 0xffffffff)) & (UCT_HASH_SIZE - 1)
...
class NodeHash:
    ...
    # 未使用のインデックスを探して返す
    def search_empty_index(self, hash_code, color, moves):
        key = hash_to_index(hash_code)
        i = key

        while True:
            if not self.node_hash[i].flag:
                self.node_hash[i].hash = hash_code
    ...
    # ハッシュ値に対応するインデックスを返す
    def find_same_hash_index(self, hash_code, color, moves):
        key = hash_to_index(hash_code)
        i = key
        ...
        while True:
            if not self.node_hash[i].flag:
                return UCT_HASH_SIZE
            elif self.node_hash[i].hash == hash_code and self.node_hash[i].color == color and self.node_hash[
                i].moves == moves:
                return i
            i += 1
            if i >= UCT_HASH_SIZE:
