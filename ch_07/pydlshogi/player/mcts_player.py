...
class MCTSPlayer(BasePlayer):
    def __init__(self):
        super().__init__()
        # モデルファイルのパス
        self.modelfile = 'model/model_policy_value_resnet'
        self.model = None  # モデル
    ...
    def select_max_ucb_child(self, board, current_node):
        ...
        q = np.divide(child_win, child_move_count, out=np.repeat(np.float32(0.5), child_num),
    ...
    def isready(self):
        ...
        if self.model is None:
            self.model = PolicyValueResnet(5)
            # self.model.to_gpu()
